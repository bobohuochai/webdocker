...
import { getTargetValue } from '../src/common';

describe('getTargetValue', () => {
  it('should work well', () => {
    const ret1 = getTargetValue(window, undefined);
    expect(ret1).toEqual(undefined);

    const ret2 = getTargetValue(window, null);
    expect(ret2).toEqual(null);

    const fn1 = getTargetValue(window, function bindThis(this: any) {
      return this;
    });
    const a3returns = fn1();
